...
//
//   node scripts/hwpack.ts hilt <file.glb|file.gltf|file.obj> --name "Exar Kun" --creator "Name" --licence "All rights reserved" [--id exar-kun] [--fit '{"rollDeg":90}'] [--finish 'Opaque(196,197,196)=chrome;Opaque(75,75,75)=anodised'] --out ../../packs
...
//
// A GLB keeps its own metal and roughness; an OBJ takes the .mtl beside it and has its finishes guessed from colour.
// --finish overrides either, by material name (the packer prints them). --copy allows copying a font to a card; without it the app only plays it.

import { readFileSync, writeFileSync, readdirSync, statSync, mkdirSync } from 'node:fs';
import { basename, dirname, join, extname } from 'node:path';
import { writePack, readManifest, objToMesh, gltfToMesh, encodeMesh, type PackManifest } from '../src/main/hwpack.ts';
...
if (mode === 'hilt') {
  ...
  const name = opts.name ?? basename(modelPath, extname(modelPath)).replace(/_Hiltwright$/i, '');
  ...
  const mesh = ext === '.glb' || ext === '.gltf'
    ? gltfToMesh(readFileSync(modelPath), { finishes, readUri: (uri) => readFileSync(join(dirname(modelPath), uri)), warn: (note) => console.log(`  note: ${note}`) })
    : objToMesh(readFileSync(modelPath, 'utf8'), (() => { try { return readFileSync(opts.mtl ?? modelPath.replace(/\.obj$/i, '.mtl'), 'utf8'); } catch { return ''; } })(), finishes);
  ...
  console.error('usage: hwpack.ts hilt <file.glb|file.obj> | font <folder> | show <file.hwpack>');
  process.exit(2);
}
