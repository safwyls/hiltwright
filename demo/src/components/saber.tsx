...
export function Lens({ color, d = 22, lit = true, pulse }: { color: string; d?: number; lit?: boolean; pulse?: boolean }) {
  return <span className={`lens ${lit ? '' : 'off'} ${pulse ? 'pulse' : ''}`} style={{ '--c': color, width: d, height: d } as CSSProperties} aria-hidden="true" />;
}

export function Spinner({ d = 34, on = true }: { d?: number; on?: boolean }) {
  return (
    <span className={`spinner ${on ? 'on' : ''}`} style={{ width: d, height: d }} aria-hidden="true">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.7} strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="7.5" opacity=".35" /><path d="M12 4.5a7.5 7.5 0 017.5 7.5" /><path d="M17 8.5l2.5 3.5 3-3" />
      </svg>
    </span>
  );
}
...
/** Small glyph for a blade, by role. */
export function BladeGlyph({ blade, color, lit = true, spinning }: { blade: Blade; color: string; lit?: boolean; spinning?: boolean }) {
  switch (blade.role) {
    case 'crystal': return <Crystal color={color} h={22} lit={lit} />;
    case 'accent': return <Lens color={blade.type === 'single' ? '#ffffff' : color} d={14} lit={lit} />;
    case 'motor': return <Spinner d={20} on={!!spinning} />;
    case 'side': return <span className="row" style={{ gap: 2 }}><span className="blade thin" style={{ '--c': color, width: 10, borderRadius: '3px 0 0 3px', opacity: lit ? 1 : 0.2 } as CSSProperties} /><span className="blade thin" style={{ '--c': color, width: 10, opacity: lit ? 1 : 0.2 } as CSSProperties} /></span>;
    default: return <span className="blade" style={{ '--c': color, width: 22, height: 6, borderRadius: '0 3px 3px 0', opacity: lit ? 1 : 0.2 } as CSSProperties} />;
  }
}

export function RoleIcon({ role }: { role: Blade['role'] }) {
  const map: Record<Blade['role'], string> = { main: 'blade', crystal: 'crystal', accent: 'led', side: 'side', motor: 'motor' };
  return <Icon name={map[role]} />;
}
