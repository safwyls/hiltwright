...
> Verified 2026-09-13 with a first-run install from an empty folder: arduino-cli 1.3.1 zip, `core install proffieboard:stm32l4` (GCC 308 MiB), ProffieOS v8.10 zip; about 2.0 GB on disk. **Windows path length bites:** with the toolchain root 139 characters deep, GCC's `cc1plus.exe` sat at 259 characters and `arm-none-eabi-g++` failed with `cannot execute 'cc1plus': CreateProcess: No such file or directory`. Hiltwright keeps the toolchain under `%LOCALAPPDATA%\Hiltwright	oolchain` and refuses roots longer than 120 characters. **Second finding, 2026-09-20, from a clean PC:** that was not the binding limit. GCC opens its own headers through an un-normalised path (`bin/../lib/gcc/arm-none-eabi/14.2.1/../../../../arm-none-eabi/include/c++/14.2.1/arm-none-eabi/thumb/v7e-m+fp/hard/bits/error_constants.h`), 213 characters below the root, so the root may be 46 characters at most. `%LOCALAPPDATA%` plus any real user name is already too long (the clean PC failed at 262 with `system_error:41: fatal error: bits/error_constants.h: No such file`; the spike cache had been sitting at exactly 259). The default root is now `%ProgramData%/Hiltwright` (25 characters), roots over 40 are refused, and the build error is translated.
