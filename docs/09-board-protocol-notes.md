...
- A recorded transcript from an OS 8 board is still needed for the `tag|command` framing and for `list_named_style` / `describe_named_style` output. Until then the client works untagged, which is the mode every existing saber needs anyway.
