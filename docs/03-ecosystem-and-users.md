...
> Import status, 2026-09-20. **Xenopixel (XENO3) is implemented** from what the vendors and tools document: numbered font folders `1`..`34`, each with `fontconfig.ini` holding one line `Name=(R,G,B),A,B,C,D,E,F,G,H` (colour 0-255; A blade effect 0 fire, 1 steady, 2 unstable, 3 rainbow, 4 candy, 5 crack, 6 pulse, 7 flashing; B blaster, C force, D lock-up, E clash effects; F blade style; G ignition and H retraction speed, defaults 200 and 500), sounds named `hum (1).wav`, music as `track (1).wav`, system sounds under `setting/`. Names that differ from ProffieOS: blaster/blst, clash/clsh, swing/swng, begindrag/bgndrag, beginlock/bgnlock, beginmelt/bgnmelt, postoff/pstoff, beginlightingblock/bgnlb, endlightingblock/endlb, poweron/boot; `in` and `out` mean the same on both. Code: `packages/core/src/xenopixel.ts`, `apps/desktop/src/main/importer.ts`, the panel on Fonts & SD. Blade effects are firmware features of that board and do not carry over; the colour does. Sources: https://sybersabers.com/en/pages/xenov3_setting , https://github.com/TyanColte/xenopixel-manager (MIT), https://github.com/NoSloppy/SoundFontNamingConverter5 (GPL-3.0; only the published name differences were used, no code).
>
> **CFX is not implemented**, because its file syntax is not published on the web pages we could read. Known: `colors.txt` in the card root with up to 32 profiles whose keys are `color`, `fcolor` (flash on clash), `bcolor` (blaster), `lcolor` (lockup), `kcolor` (flicker), `tcolor` (stab), `dcolor` (drag), `xcolor` (crystal chamber), each `R,G,B,W` in 0-1023; `config.txt` blade profiles with `pname`, `ledstrip`, `style_*`, `flks` and so on; per-font `font_config.txt` with `start_blade`, `start_color`. Unknown: how profiles are delimited and the exact `key=value` form. That is on page 53 of the Crystal Focus manual shipped on every CFX card, so a real card (or that PDF) is needed before writing a parser.
